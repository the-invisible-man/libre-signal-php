...
| --- | --- |
| `setAtWithTtl("A", "B", "C", 1, 10)` | returns ""; database state: `{"A": {"B": "C"}}` with lifespan `[1, 11)`, meaning that the record should be deleted at timestamp = 11. |
| `backup(3)` | returns "1"; saves the database state |
| `setAt("A", "D", "E", 4)` | returns ""; database state: `{"A": {"D": "E", "B": "C"}}` |
| `backup(5)` | returns "1"; saves the database state |
| `deleteAt("A", "B", 8)` | returns "true"; database state: `{"A": {"D": "E"}}` |
...
| `setAt("B", "C", "D", 11)` | returns ""; database state: `{"A": {"D": "E", "B": "C"}, "B": {"C": "D"}}` |
| `backup(12)` | returns "2" because we have two keys in the database; saves the database state |
| `scanAt("A", 15)` | returns "B(C), D(E)" |
| `scanAt("A", 16)` | returns "D(E)" |
| `scanAt("B", 17)` | returns "C(D)" |

## Test
You can execute the test cases for this level by running the following command in the terminal: `vendor/bin/phpunit --testsuite in_memory_database --group level4` from the project root directory.
